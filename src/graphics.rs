...
const DIMS: window::Extent2D = window::Extent2D { width: 640, height: 640 };
...
where
    B: gfx_hal::Backend,
{
    pub fn new(instance: Option<B::Instance>, window: winit::window::Window) -> Renderer<B> {
        ...
        for x in 300..=340 {
            for y in 300..=340 {
                ...
                img.put_pixel(y, x, image::Rgba([0, 0, 0, 255]));
            }
        }
        ...
        let pipeline = {
            let vs_module = {
                let spirv =
                    pso::read_spirv(Cursor::new(&include_bytes!("../data/quad.vert.spv")[..]))
                        .unwrap();
